...
export default function SceneLibrary() {
  ...
  const tab: SceneTabKey =
    tabParam === 'review' || tabParam === 'generating' || tabParam === 'failed'
      ? tabParam
      : 'library';
  const [loading, setLoading] = useState(false);
  const [items, setItems] = useState<SceneLibraryItem[]>([]);
  const [filters, setFilters] = useState<SceneLibraryFilters>({
    brands: [],
    spaces: [],
    styles: [],
    scene_names: [],
  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      >
        <Title level={4} style={{ margin: 0 }}>
          ...
          <Badge
            count={items.length}
            overflowCount={9999}
            style={{ marginLeft: 10, backgroundColor: '#1677ff' }}
          />
        </Title>
        <Space wrap>
          <Select
          ...
          <Button icon={<SyncOutlined />} onClick={fetchItems}>
            刷新
          </Button>
        </Space>
      </div>
      ...
      <div style={{ marginBottom: 14, color: '#666', fontSize: 13 }}>
        ...
        {tab === 'review' &&
          '这里展示已生成完成但尚未入库的场景图，包括 agent 在客户对话中生成的结果。'}
              ...
              <Col key={item.id} xs={24} sm={12} md={8} lg={6} xl={6}>
                ...
                >
                  ...
                  <Space size={4} wrap>
                    ...
                    {item.style_hint && (
                      <Tag color={tagColor(item.style_hint)}>{item.style_hint}</Tag>
                    )}
                  </Space>
                  {item.request_text && (
                    <div style={{ marginTop: 8, minHeight: 34 }}>
                      <Text
                        type="secondary"
                        ellipsis={{ tooltip: item.request_text }}
                        style={{ fontSize: 12 }}
                      >
                        {item.request_text}
                      </Text>
                  ...
                  <div style={{ marginTop: 8, fontSize: 12, color: '#888' }}>
                    {item.duration_ms > 0
                      ? `耗时 ${(item.duration_ms / 1000).toFixed(1)} 秒`
          ...
          <div>
            <div style={{ marginBottom: 16 }}>
              <Space wrap style={{ marginBottom: 8 }}>
                ...
                {selected.primary_product_brand && (
                  <Tag color="blue">{selected.primary_product_brand}</Tag>
                )}
              </Space>
            ...
            <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginBottom: 14 }}>
                ...
                <div>
                  <Text>
                    {selected.duration_ms > 0
                      ? `${(selected.duration_ms / 1000).toFixed(1)} 秒`
                      : '进行中'}
                  </Text>
                </div>
              </div>
              ...
              {selected.conversation_id && (
                <div>
                  ...
                  <div>
                    <Tag color="geekblue">来自对话 #{selected.conversation_id}</Tag>
                  </div>
                </div>
              )}
            </div>
